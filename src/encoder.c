...
void encoder0() {
	if(digitalRead(encoder[0].pin2) == HIGH) {
        encoder[0].value++;
    }
	else {
        encoder[0].value--;
...
void encoder1() {
	if(digitalRead(encoder[1].pin2) == LOW) {
        encoder[1].value++;
    }
	else {
        encoder[1].value--;
...
void encoder2() {
	if(digitalRead(encoder[2].pin2) == HIGH) {
        encoder[2].value++;
    }
	else {
        encoder[2].value--;
...
void encoder3() {
	if(digitalRead(encoder[3].pin2) == LOW) {
        encoder[3].value++;
    }
	else {
        encoder[3].value--;
